...
| **Compile** | Всё для одной компиляции: request в _compile_kernel, module, args, grid, thread_tensor_indices, опции; source info по тредам. | `KernelCompileRequest`, `TTNNKernelCompileRequest`, `ThreadConfigBuildRequest`, `KernelWriteRequest`, `TTNNKernelCompileOptions`, `ThreadSourceInfo`; `ttl.compile.pipeline`: `_compile_kernel` (оркестрация); `ttl.compile.thread_compiler`, `ttl.compile.ttnn_compiler`, `ttl.compile.kernel_writer`, `ttl.compile.source_collector`; `_collect_source_info_from_threads` в source_collector; результат — MLIR module, kernel paths, `CompiledTTNNKernel`. Типы дескрипторов в пакете `ttl.descriptor_options` (compile_request, compile_result, thread_config, program_config, core_range). |
...
| TTNNKernelCompileRequest | CompiledTTNNKernel | Валидация request (тензоры TTNN, число ядер) — в TTNNKernelCompileRequest (model_validator). `ttl.compile.ttnn_compiler.compile_ttnn_kernel(req)`. |
...
Входы и выходы трансформаций — только Pydantic-типы или типы из ttnn_proxy. Логика компиляции: `ttl.compile.pipeline` (_compile_kernel — оркестрация), `ttl.compile.thread_compiler`, `ttl.compile.ttnn_compiler`, `ttl.compile.kernel_writer`, `ttl.compile.source_collector`; логика рантайма: `ttl.runtime.runner`; типы на границе с ttnn/MLIR: `ttl.boundary` (ttnn_types, mlir_types). API: `run(program, *args, grid=...)` и `ProgramBuilder` (ttl.program.builder) для упрощённого запуска и сборки ProgramSpec. Контракты run: RunRequest (num_outs == 1), декоратор: ProgramDecoratorParams (grid обязателен, indexing_maps при iterator_types). Согласованность indexing_maps и iterator_types (число параметров каждого indexing_map = len(iterator_types)) проверяется в ProgramDecoratorParams.model_validator; в ttl_api ад-hoc проверок нет.

## 5. Реестр Python-диалектов

- **Program**: ProgramOptions, ProgramDecoratorParams, ProgramConfig, KernelCompileRequest, ProgramSpec, RunRequest, ProgramBuilder (иерархия: root + options: ProgramOptions). ProgramBuilder — fluent builder для ProgramSpec.
- **Graph**: OpGraph, OpNode, Topology, SchedulePlan.
- **Compile**: KernelCompileRequest, TTNNKernelCompileRequest, ThreadConfigBuildRequest, KernelWriteRequest, TTNNKernelCompileOptions, ThreadSourceInfo; `ttl.compile.pipeline`: `_compile_kernel` (оркестрация); `ttl.compile.thread_compiler`, `ttl.compile.ttnn_compiler`, `ttl.compile.kernel_writer`, `ttl.compile.source_collector`; `_collect_source_info_from_threads` в source_collector; валидация в `ttl.compile.validation`; TTLGenericCompiler.source_info → ThreadSourceInfo. Типы дескрипторов: пакет `ttl.descriptor_options` (compile_request, compile_result, thread_config, program_config, core_range).
...
Внутренние типы на границах слоёв приведены к Pydantic: ThreadWrapperView (ttl.compile.source_collector), CompilerContext (ttl._src.context), KernelSpec (ttl.runtime.runner), TTNNLayoutConfig (layouts) — все BaseModel; ProgramSpec и run(spec, *args) или run(program, *args, grid=...) — фасад идеального UX (см. 18_ideal_ux_and_layer_responsibilities.md). Модуль ttl.layers реэкспортирует типы по слоям для одного фокуса при чтении. Примитивы (ttl.primitives): BuildContext, ComposablePrimitive, DMPrimitive, Reader, Writer, ComputePrimitive, ElementwiseOp, MatmulOp, SyncPrimitive, Semaphore, PrimitiveRegistry — для сборки программ из элементарных блоков. Тесты по слоям: test_dialect_layers.py (program/compile/runtime), test_program_builder.py (ProgramBuilder), test_primitives_layer.py (primitives).

Реестр исполнительных Python-диалектов ведётся отдельно от документационного пайплайна (Doc–MLIR–GraphDB, диалект ttm.sdlc_doc). Регистрация: в этом документе и при необходимости в **`.cursor/artifacts_mlir_graphdb`** (канон в репо); опционально `docs/sdlc/_KG_MLIR` в портфолио-репо для прослеживаемости.
