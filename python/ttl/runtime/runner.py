...
"""
Runtime layer implementation: compilation artifacts -> descriptors -> run_kernel_on_device.

Runtime layer: KernelSpec, CB configs, CoreRangeSet -> ttnn descriptors -> execution.
...
class KernelSpec(BaseModel):
    ...
    config: object = Field(..., description="Kernel config descriptor (ttnn)")
...
class KernelDescriptorBuildRequest(BaseModel):
    ...
    tensors: list[TtnnTensorLike] = Field(
        ..., description="ttnn.Tensor objects; position = global index"
    )
    tensor_accessor_args: list[int] = Field(
        ..., description="Flattened compile-time args from all tensors"
    )
    core_ranges: CoreRangeSetLike = Field(
        ..., description="ttnn.CoreRangeSet for kernel execution"
    )
...
class CBDescriptorBuildRequest(BaseModel):
    ...
    tensors: list[TtnnTensorLike | None] = Field(
        ...,
        description="ttnn.Tensor objects; position = CB index (None for intermediate)",
    )
    cb_configs: list[object] = Field(
        ..., description="CircularBuffer objects per CB index"
    )
    core_ranges: CoreRangeSetLike = Field(
        ..., description="ttnn.CoreRangeSet for CB allocation"
    )

    @model_validator(mode="after")
    def cb_configs_no_none(self) -> Self:
        """Contract: all CB indices must have associated CircularBuffer configurations."""
...
class RunKernelRequest(BaseModel):
    ...
    kernel_specs: list[KernelSpec] = Field(..., description="Kernel specifications")
    tensors: list[TtnnTensorLike] = Field(..., description="ttnn.Tensor objects")
    cb_configs: list[object] = Field(
        ..., description="CircularBuffer objects per CB index"
    )
    core_ranges: CoreRangeSetLike = Field(
        ..., description="ttnn.CoreRangeSet for execution"
    )
